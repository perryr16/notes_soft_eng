...
const BadgrService = {
  ...
  // NOTE: This function is only called from an inactive Cron: cronBadgrAssertions()
  ...
  async setBadgrAuthToken() {
    try {
      ...
      console.log(`[${new Date().toISOString()}] BADGR_AUTH_TOKEN set`);
    } catch (err) {
      console.log(`[${new Date().toISOString()}] Error setting BADGR_AUTH_TOKEN: ${err}`);
    }
